...
完整工程中的 B-24 数据包包含 104 个语义节点、5 个渐进等级、16 个生产工序、22 个运行阶段、4 台独立发动机和 10 类签名资源。模型持续使用一份不可变 GLB。在线传输支持完整对象与 6 个逐块校验的恢复分块。

## 当前分支用途

`agent/psar-v0.6-bootstrap` 用于把 PSAR 注册表、机型数据、技能和依赖接入现有 Haihao 多生产线仓库。现有 `architecture-pipeline` 与 `dem-pipeline` 不受影响。

当前分支已经包含签名注册表入口、三个通道、七机型目录、B-24 运行配置、生产工序、运行特效、烟雾、材质、资料登记、飞机生产技能、依赖锁和技术来源登记。

当前分支仍属于待审查引导分支。104 节点完整语义骨骼、网页应用源码、可信元数据全套文件、23,085,972 字节 B-24 GLB 与 6 个二进制恢复分块保存在完整 v0.6 发布包中，需要通过项目自带的认证发布脚本或 Cloudflare R2 工作流上传并完成远端哈希复验后再合并。发布私钥不得提交到仓库。
